...
def training_examples(vocab_words, vocab_tags, gold_data, parser, batch_size=100):
    for data in gold_data:
      index = 0
      batch = torch.zeros(((len(data)*2-1), 3))
      ...
      for i in range((2*len(gold_heads))-1):
        pos_moves = oracle_moves_dynamic(gold_heads, parser, config)
        feat = parser.featurize(rnn_embeddings, config)
        feat = feat.unsqueeze(0)
        output = parser.model.forward(feat)
...
def train_fixed_window(train_data,vocab_words, vocab_tags, n_epochs=1, batch_size=100, lr=1e-2):
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(total=12544) as pbar:
          for output, moves in training_examples(vocab_words, vocab_tags, train_data, parser, batch_size):
              optimizer.zero_grad()
